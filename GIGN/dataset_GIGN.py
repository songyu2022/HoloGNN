...
def inter_graph(ligand, pocket, dis_threshold = 5.):
    ...
    #print(f'dis_matrix is {dis_matrix.shape}')
    node_idx = np.where(dis_matrix < dis_threshold)
    #print(f'node_idx is {node_idx.shape}')
    ...
    #print(f'edge_index_inter is {edge_index_inter.shape}')
    #exit()
...
if __name__ == '__main__':
    data_root = './data'
    # toy_dir = os.path.join(data_root, 'external_set')
    # toy_df = pd.read_csv(os.path.join(data_root, "external.csv"))
    toy_dir = os.path.join(data_root, 'PDB_test2013_107')
    toy_df = pd.read_csv(os.path.join(data_root, "test2013_107.csv"))
